...
class DetMobileNetV3(BaseBackbone):
    def __init__(self, mode, in_channels, scale=1, disable_se=False, num_classes=1000):
        ...
        for idx, (k, input, inner, out, se, act, stride) in enumerate(cfg):
            ...
            # 这个操作: large有四次s=2,取后三次
            # 这里注意，每次都是在stride之前取特征输出，为了第一个h.w对应1/4的feature map
            start_idx = 2 if mode == 'large' else 0
            if stride == 2 and i > start_idx:
                # self.out_channels.append(in_planes)
